...
setLanguageSelector: function(){
  ...
  */
  ...
  slideshare_object.prompt_connect_li_and_ss = function(){
    if (slideshare_object.user.show_li_connect_cta){
      bootbox && bootbox.confirm("Share your SlideShare likes with linkedin network?", function(confirmed){
      if (confirmed) {
        var config = {
        'connectButtonClass': ".j-linkedin-connect",
        'state': "12314143",
        'login_source': "favorites",
        'connectRoute' : "bind_account"
        };
        ssliconnect = new LIConnect(config);
        ssliconnect.openAuthDialog();
        window._gaq.push(['_trackEvent', "linkedin", "like_cta_shown"]);
      }
    });
    }
  };
        ...
        if(response.success){
          ...
          if(config.is_favorited){
            slideshare_object.post_activity_to_facebook && slideshare_object.post_activity_to_facebook(
              { name: 'favorite' , ppt_url: config.ss_url }, function(response){});
            slideshare_object.isHomePage() && slideshare_object.prompt_connect_li_and_ss();
          }
